...
- agent-facingなMCP toolは、レジストリ概要取得、検索、取得、draft作成/編集、更新提案、archive推薦、レビュー状況確認、履歴取得までにする（10 tools）。archive推薦（`recommend_archive`）と履歴取得（`get_note_history`）もMVPのagent-facing toolに含める
...
合計10 tools。`recommend_archive`（archive推薦）は内容変更を伴わない提案として、`propose_note_update`と同じcontribution planeに置く。`get_note_history`（監査・文脈用の変更履歴取得。snapshotは含まない軽量版）はreview planeに置き、before/afterのsnapshotを含む詳細な差分が必要な場合はCLIの`agentpress history <id>`を案内する。
...
```json
{
  ...
  "usage_policy": "verified のみ正式根拠として使う。stale: true のnoteは要再確認として扱い、回答時にその旨を明示する。関連するverified noteが見つからない場合はcreate_note_draftで新規知識案を提案する。古くなった/もう使うべきでないverified noteを見つけた場合は、recommend_archiveで人間にarchiveを提案する。",
...
- どちらも履歴を残す（`archive_recommendation`の承認は`note_archived` + `proposal_approved`を記録する）
...
ノートを`archived`にするCLI操作。AIが`recommend_archive`で提案したarchive_recommendation proposalを承認する（`agentpress approve <proposal_id>`）のも、内部的には同じarchive操作を実行する経路になる。
...
- SQLite
- Markdown import/export
- MCP: registry overview/search/read/create draft/update draft/propose update/archive推薦/review status確認/履歴取得（10 tools）
...
- MCP toolとして `get_registry_overview` / `search_notes` / `get_note` / `create_note_draft` / `update_draft` / `propose_note_update` / `recommend_archive` / `list_review_items` / `get_review_item` / `get_note_history` が使える（10 tools）
